// var app = angular.module("myProject", ["ngRoute"]);
        ...
        app.run(function($rootScope, $http)
        {
            ...
            $http.get("fontend/JSON/product.json").then(function(response)
            {
                $rootScope.plist = response.data.productList;   
            })
            $http.get("fontend/JSON/user.json").then(function(response)
            {
                $rootScope.list = response.data.userList;
            })
            $http.get("fontend/JSON/offers.json").then(function(rsp)
            {
                $rootScope.offers = rsp.data.offersList;
            })
            $http.get("fontend/JSON/city.json").then(function(rsp)
            {
                $rootScope.cities = rsp.data.cityList;
            })
            $http.get("fontend/JSON/topsale1.json").then(function(response)
            {
                $rootScope.slist = response.data.productList20;   
            })
            $http.get("fontend/JSON/topsale.json").then(function(response)
            {
                $rootScope.newlist = response.data.productList2;   
            })
            $http.get("fontend/JSON/dryfruits.json").then(function(response)
            {
                $rootScope.dlist = response.data.dryfruitList;   
            })
            $http.get("fontend/JSON/nuts.json").then(function(response)
            {
                $rootScope.nlist = response.data.nutList;
            })
            $http.get("fontend/JSON/seeds.json").then(function(response)
            {
                $rootScope.elist = response.data.seeds;
            })
            $http.get("fontend/JSON/combopacks.json").then(function(response)
            {
